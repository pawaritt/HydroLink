...
const channel2 = new BroadcastChannel(data.channelcode)
...
function AddProduct(title, slug, alert = true) {
    if (!(products.includes(slug))) {
        products.push(slug)
        container = document.getElementById('product-container');
        container.innerHTML += `
        <a id="remove-product">
            <div class="btn btn-square btn-outline-light btn-sm" id="added-product" value="${slug}" style="border-style:dashed;">
                ${title}  &times;
            </div>
        </a> `
...
$(document).ready(function () {
    channel1.addEventListener("message", (e) => {
        AddProduct(e.data.title, e.data.slug)
    });
    $('#add-product').click(function () {
        elem = $("#selectproduct :selected");
        console.log('click')
        AddProduct(elem.attr('name'), elem.val(this));
    });
    $('form[id="collection"]').submit(function (e) {
        e.preventDefault();
        var requestData =  new FormData(this);
        requestData.set('products', products);
